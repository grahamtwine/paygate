...
import za.co.payhost.model.payweb3.PayWeb;
...
public abstract class PayHostWindow extends JFrame{
	...
	protected transient PayWeb payWeb;
    protected transient PayGateFormProperties formData = null;
	...
	protected PayHostWindow(String title, String endpoint) {
		payWeb = new PayWeb(endpoint);
	...
	protected JTextField createTextField( String name ) {
    	...
    	tf.setName(name);
    	tf.setMaximumSize(UIUtils.TXTFIELD_DIMENSION);
	...
	protected void updateRequestField(KeyEvent evt) {
    	...
    	formData.setProperty(txtField.getName(), txtField.getText().trim());
    ...
    protected void sendRequest() {
        ...
        try {
        	LOG.log(Level.INFO, ".:..::Form Data::..:..\n{0}\n::.::..:.::..:", formData);
        	String request = payWeb.body(formData);
        	
        	LOG.log(Level.INFO, ".:..::Sending::..:..\n{0}\n::.::..:.::..:", request);
            String response = payWeb.request(request);
    ...
    protected void addMenuItem(JLabel label, JTextField textField) {
    	menu.add(label);
    	menu.add(textField);
    }
